...
public class OrderPanelController {
    ...
    public String singleOrder(@PathVariable Long id, Model model) {
        ...
        return order.map(ord -> singleOrderPanel(ord, model)).orElse("redirect:/");
    }

@PostMapping("panel/order/{id}")
    public String changeOrderStatus(@PathVariable Long id, Model model) {
        Optional<Order> order = orderRepository.findById(id);
        order.ifPresent(ord -> {ord.setStatus(OrderStatus.nextStatus(ord.getStatus()));
    ...
    private String singleOrderPanel(Order order, Model model) {
        ...
        model.addAttribute("sum", order.getProducts().stream().mapToDouble(Product::getPrice).sum());
        return "panel/order";
    }
